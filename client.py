...
def leftBoard():
    ...
    for box in range(11):
        if box == 0:
            boxLabel = Label(gameWindow, font = ("Helvetica", 30), width = 2, height=1, borderwidth=0, bg="red")
            boxLabel.place(x=xPos, y=screen_height/2 - 80)
            leftBoxes.append(boxLabel)
            xPos+=50
        else :
            boxLabel = Label(gameWindow, font = ("Helvetica", 30), width = 2, height=1, borderwidth=0, bg="white")
            boxLabel.place(x=xPos, y=screen_height/2 - 80)
            leftBoxes.append(boxLabel)
            xPos+=50
...
def rightBoard():
    ...
    for box in range(11):
        if box == 10:
            boxLabel = Label(gameWindow, font = ("Helvetica", 30), width = 2, height=1, borderwidth=0, bg="yellow")
            boxLabel.place(x=xPos, y=screen_height/2 - 80)
...
def askPlayerName():
    ...
    canvas1.create_text( screen_width/2 + 50, screen_height/5, text = "Enter Name", font=("Chalkboard SE",100), fill="white")

    nameEntry = Entry(nameWindow, width=15, justify='center', font=('Chalkboard SE', 50), bd=5, bg='white')
    nameEntry.place(x=screen_width/2 - 250, y = screen_height/3)


    button = Button(nameWindow, text="Save", font=("Chalkboard SE", 30),width=15, command=saveName, height=2, bg="#80deea", bd=3)
    button.place(x=screen_width/2 - 150, y = screen_height/2)
...
def recivedMsg():
    ...
    while True:
        ...
        if ("player_type" in message): #player_type
